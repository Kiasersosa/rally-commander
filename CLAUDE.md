...
- **Phase 10 (Notifications):** shipped (code) — Twilio secrets pending. `users.phone_number` + `users.sms_opt_in` columns; `notifications` audit table (channel email/sms × kind digest/expiry_alert/bulletin/manual × status pending/sent/failed). **`NotificationDigestComposer`** (src/lib/notification-digest-composer.ts) is the **6th deep module** — pure function `(user, period, todos, events, expirations, documents) → digest` with smart subject-line generation (urgent items bubble up). 10 Vitest cases. Adapters: Resend for email (existing), Twilio for SMS (lazy-imported, gracefully reports "not configured" when secrets missing). `recordAndSend` writes a `pending` row to the audit log first then updates to `sent`/`failed`. Cron entrypoints: `/api/digest/run` POST (weekly) and `/api/expiry-alerts/run` POST (daily) — both require `Bearer ${CRON_SECRET}` auth. GitHub Actions workflows (`.github/workflows/cron-digest.yml`, `cron-expiry-alerts.yml`) hit them on schedule. Profile editor at `/profile` lets each user manage phone + SMS opt-in.
